...
export function Pages({ max, value, onChange, M = 3 }
  : { max: number, value: number, onChange: (value: number) => void, M?: number }
) {
  ...
  return (
    <div className="w-full flex justify-center my-4">
      ...
      {midRight < max - 2 && <button
        className="bg-slate-200 px-4 py-1 text-slate-500 font-medium "
        disabled>...</button>}
      {max > 1 && (
        <button className="bg-slate-200 px-4 py-1 text-slate-500 font-medium rounded-r-lg disabled:bg-slate-300 text-slate-600" onClick={() => onChange(max)}
          disabled={value === max}>
          {max}
        </button>
      )}
