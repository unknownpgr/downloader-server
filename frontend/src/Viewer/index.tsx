...
export function Viewer() {
  ...
  if (["mp4", "webm", "ogg", "mkv", "ts"].includes(ext)) {
    viewerComponent = (
      <video
        src={src}
    ...
    viewerComponent = (
      <img src={src} alt="img" className="max-w-full max-h-full" />
    );
  } else if (["pdf"].includes(ext)) {
    viewerComponent = (
      <iframe src={src} className="max-w-full max-h-full"></iframe>
    );
    ...
    viewerComponent = (
      <div className="max-s-lg bg-white p-4 rounded-lg shadow-lg">
        Not supported file type <code>{src.split(".").pop()}</code>.
      </div>
    );
  }
